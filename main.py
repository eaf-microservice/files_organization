...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
                             QPushButton, QLabel, QFileDialog, QTextEdit, QMessageBox, QDialog)
                ...
                if os.path.isfile(file_path):
                    ...
                    for folder, extensions in file_types.items():
                        if file_extension in extensions:
                            ...
                            self.progress.emit(f"Moved: {file_name} → {folder}")
                            moved = True
                            files_moved += 1
                            break
                    ...
                    if not moved:
                        ...
                        shutil.move(file_path, dest_path)
                        self.progress.emit(f"Moved: {file_name} → Others")
                        files_moved += 1
...
class MainWindow(QMainWindow):
    ...
    def organize_files(self):
        ...
        self.organizer_thread.progress.connect(self.update_status)
